...
import { useRoqClient, useGameFindManyWithCount } from 'lib/roq';
...
export function GameListPage(props: GameListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['name.contains', 'publisher.contains', 'genre.contains'],
          ...params.filters,
        },
        'game',
      ),
  ...
  const { data, error, isLoading, mutate } = useGameFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.game.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('card', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Card',
          accessorKey: 'card',
          cell: ({ row: { original: record } }: any) => record?._count?.card,
        }
      : null,
