...
import { useRoqClient, useGameFindFirst } from 'lib/roq';
...
function GameEditPage() {
  ...
  const { data, error, isLoading, mutate } = useGameFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: GameInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.game.update({
        data: values as RoqTypes.game,
        where: {
          id,
        },
      });
